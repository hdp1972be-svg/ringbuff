...
rb_err_t rb_publish_ex(rb_t *rb, uint32_t slot_index, uint32_t written_len, bool truncated) {
    ...
    /* Make CPU writes to this slot visible to a device (GPU/FPGA/DMA/…)
     * before we publish ownership.  No-op on coherent DRAM. */
    RB_HW_FLUSH_SLOT(rb, slot_index);
#if RB_PER_SLOT_LAP
...
rb_err_t rb_consume(rb_t *rb, uint32_t *out_slot_index, const void **out_obj, uint32_t *out_len,
    ...
    /* Ensure CPU can see device writes to this slot (GPU/FPGA/DMA/…).
     * No-op on coherent DRAM. Must happen before we hand the pointer out. */
    RB_HW_INVALIDATE_SLOT(rb, slot_index);
