...
typedef enum { RB_OK = 0, RB_ERR_FULL, RB_ERR_EMPTY, RB_ERR_OVERSIZE, RB_ERR_INVAL, RB_ERR_NOT_INIT } rb_err_t;
...
typedef void (*rb_slot_cb_t)(rb_t *rb, uint32_t slot_index, uint32_t len, bool truncated, void *user);
typedef struct { rb_slot_cb_t on_slot_added; rb_cb_t on_low_d; rb_cb_t on_low_e; rb_cb_t on_full; void *user; } rb_callbacks_t;
#if RB_ENABLE_STATS
typedef struct { uint64_t published; uint64_t consumed; uint64_t truncated; uint64_t full_attempts; uint32_t high_water; uint32_t full_hits; uint32_t low_d_hits; uint32_t low_e_hits; } rb_stats_t;
#endif
typedef struct {
    uint32_t capacity; uint32_t limit; uint32_t slots; uint32_t slot_size;
    uint32_t low_d; uint32_t low_e; rb_oversize_policy_t oversize_policy;
    size_t producer_stack_size; size_t consumer_stack_size; rb_callbacks_t cb;
} rb_config_t;
...
typedef bool (*rb_flush_fn)(void *writable, uint32_t cap, uint32_t *out_len, bool *out_truncated, bool *out_stop, void *user);
